...
class EntityEventHandler implements Listener
{
	...
	public void onEntityChangeBlock(EntityChangeBlockEvent event)
	{
		...
		else if(!GriefPreventionPlus.instance.config_silverfishBreakBlocks && event.getEntityType() == EntityType.SILVERFISH)
		{
			event.setCancelled(true);
		}
		...
		else if(event.getEntityType() == EntityType.WITHER && GriefPreventionPlus.instance.config_claims_worldModes.get(event.getBlock().getWorld()) != ClaimsMode.Disabled)
		{
			event.setCancelled(true);
		}
		...
		else if (event.getEntityType() == EntityType.FALLING_BLOCK)
		{
		    ...
		    {
		        entity.setMetadata("GP_FALLINGBLOCK", new FixedMetadataValue(GriefPreventionPlus.instance, block.getLocation()));
		    }
		    ...
		    else
		    {
		         ...
		         if(originalLocation.getBlockX() != newLocation.getBlockX() || originalLocation.getBlockZ() != newLocation.getBlockZ())
		         {
		             ...
		             if(GriefPreventionPlus.instance.config_claims_worldModes.get(newLocation.getWorld()) == ClaimsMode.Creative)
		             {
		                 event.setCancelled(true);
		                 return;
		             }
	...
	{		
		if(!GriefPreventionPlus.instance.config_zombiesBreakDoors) event.setCancelled(true);
	}
	...
	public void onEntityInteract(EntityInteractEvent event)
	{
	    ...
	    {
	        if(!GriefPreventionPlus.instance.config_creaturesTrampleCrops)
	        {
	            event.setCancelled(true);
	        }
	        else
	...
	{		
		...
		if(!GriefPreventionPlus.instance.creativeRulesApply(location.getWorld()))
        {
		    originationClaim = GriefPreventionPlus.instance.dataStore.getClaimAt(location, false, null);
        }
		
		if( location.getWorld().getEnvironment() == Environment.NORMAL && GriefPreventionPlus.instance.claimsEnabledForWorld(location.getWorld()) && ((isCreeper && GriefPreventionPlus.instance.config_blockSurfaceCreeperExplosions) || (!isCreeper && GriefPreventionPlus.instance.config_blockSurfaceOtherExplosions)))			
		{
			for(int i = 0; i < blocks.size(); i++)
			{
				Block block = blocks.get(i);
				if(GriefPreventionPlus.instance.config_mods_explodableIds.Contains(new MaterialInfo(block.getTypeId(), block.getData(), null))) continue;
				...
				if(block.getLocation().getBlockY() > GriefPreventionPlus.instance.getSeaLevel(location.getWorld()) - 7)
				{
					blocks.remove(i--);
				}
			}			
		...
		if(GriefPreventionPlus.instance.creativeRulesApply(explodeEvent.getLocation().getWorld()))
		{
			for(int i = 0; i < blocks.size(); i++)
			{
				Block block = blocks.get(i);
	...
	public void onItemSpawn(ItemSpawnEvent event)
	{
		...
		if(GriefPreventionPlus.instance.creativeRulesApply(event.getLocation().getWorld()))
		{
			event.setCancelled(true);
		}
		
		//if item is on watch list, apply protection
		ArrayList<PendingItemProtection> watchList = GriefPreventionPlus.instance.pendingItemWatchList;
		...
		for(int i = 0; i < watchList.size(); i++)
		{
		    ...
		    newItem.setMetadata("GP_ITEMOWNER", new FixedMetadataValue(GriefPreventionPlus.instance, pendingProtection.owner));
	...
	public void onExpBottle(ExpBottleEvent event)
	{
		...
		if(GriefPreventionPlus.instance.creativeRulesApply(event.getEntity().getWorld()))
		{
	...
	public void onEntitySpawn(CreatureSpawnEvent event)
	{
		...
		if(!GriefPreventionPlus.instance.creativeRulesApply(event.getLocation().getWorld())) return;
	...
	public void onEntityDeath(EntityDeathEvent event)
	{
		...
		if(GriefPreventionPlus.instance.creativeRulesApply(entity.getLocation().getWorld()))
		{
        ...
        boolean isPvPWorld = GriefPreventionPlus.instance.config_pvp_enabledWorlds.contains(world);
        if((isPvPWorld && GriefPreventionPlus.instance.config_lockDeathDropsInPvpWorlds) || 
           (!isPvPWorld && GriefPreventionPlus.instance.config_lockDeathDropsInNonPvpWorlds))
        {
            ...
            {
                GriefPreventionPlus.instance.pendingItemWatchList.add(
	...
	public void onHangingBreak(HangingBreakEvent event)
    {
        ...
        if(!GriefPreventionPlus.instance.claimsEnabledForWorld(event.getEntity().getWorld())) return;
        ...
        String noBuildReason = GriefPreventionPlus.instance.allowBuild(playerRemover, event.getEntity().getLocation(), Material.AIR);
	...
	public void onPaintingPlace(HangingPlaceEvent event)
	{
		...
		String noBuildReason = GriefPreventionPlus.instance.allowBuild(event.getPlayer(), event.getEntity().getLocation(), Material.PAINTING);
        if(noBuildReason != null)
        {
        	event.setCancelled(true);
		...
		else if(GriefPreventionPlus.instance.creativeRulesApply(event.getEntity().getWorld()))
		{
	...
	public void onEntityDamage (EntityDamageEvent event)
	{
        ...
        if(event.getEntity() instanceof Tameable && !GriefPreventionPlus.instance.config_pvp_enabledWorlds.contains(event.getEntity().getWorld()))
        {
		...
		if(attacker != null && event.getEntity() instanceof Player && GriefPreventionPlus.instance.config_pvp_enabledWorlds.contains(attacker.getWorld()))
		{
			...
			if(attacker != defender)
			{
				...
				if(GriefPreventionPlus.instance.config_pvp_protectFreshSpawns)
				{
				...
				if(GriefPreventionPlus.instance.config_pvp_noCombatInPlayerLandClaims || GriefPreventionPlus.instance.config_pvp_noCombatInAdminLandClaims)
				{
					Claim attackerClaim = this.dataStore.getClaimAt(attacker.getLocation(), false, attackerData.lastClaim);
					if(	attackerClaim != null && 
						(attackerClaim.isAdminClaim() && attackerClaim.parent == null && GriefPreventionPlus.instance.config_pvp_noCombatInAdminLandClaims ||
						 attackerClaim.isAdminClaim() && attackerClaim.parent != null && GriefPreventionPlus.instance.config_pvp_noCombatInAdminSubdivisions ||
						!attackerClaim.isAdminClaim() && GriefPreventionPlus.instance.config_pvp_noCombatInPlayerLandClaims))
					{
					...
					if( defenderClaim != null &&
						(defenderClaim.isAdminClaim() && defenderClaim.parent == null && GriefPreventionPlus.instance.config_pvp_noCombatInAdminLandClaims ||
				         defenderClaim.isAdminClaim() && defenderClaim.parent != null && GriefPreventionPlus.instance.config_pvp_noCombatInAdminSubdivisions ||
						!defenderClaim.isAdminClaim() && GriefPreventionPlus.instance.config_pvp_noCombatInPlayerLandClaims))
					{
		...
		if(event instanceof EntityDamageByEntityEvent)
		{
			...
			if ((subEvent.getEntity() instanceof Creature && GriefPreventionPlus.instance.config_claims_protectCreatures))
			{
			    //if entity is tameable and has an owner, apply special rules
		        if(subEvent.getEntity() instanceof Tameable && !GriefPreventionPlus.instance.config_pvp_enabledWorlds.contains(subEvent.getEntity().getWorld()))
		        {
	...
	public void onEntityDamageMonitor (EntityDamageEvent event)
	{
	    ...
	    Player defender = (Player)event.getEntity();
	    if (defender.getWorld()==null) return;
	...
	public void onVehicleDamage (VehicleDamageEvent event)
	{
		...
		if(!GriefPreventionPlus.instance.config_claims_preventTheft) return;	
		
		//input validation
		if(event.getVehicle() == null) return;
		
		//don't track in worlds where claims are not enabled
        if(!GriefPreventionPlus.instance.claimsEnabledForWorld(event.getVehicle().getWorld())) return;
    ...
    public void onPotionSplash (PotionSplashEvent event)
	{
	    ...
	    for(PotionEffect effect : effects)
	    {
	        ...
	        for(LivingEntity effected : event.getAffectedEntities())
	        {
	            ...
	            //FEATURE: prevent players from engaging in PvP combat inside land claims (when it's disabled)
	            else if(GriefPreventionPlus.instance.config_pvp_noCombatInPlayerLandClaims || GriefPreventionPlus.instance.config_pvp_noCombatInAdminLandClaims)
	            {
