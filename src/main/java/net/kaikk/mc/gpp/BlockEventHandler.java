...
public class BlockEventHandler implements Listener 
{
	...
	public void onBlockBreak(BlockBreakEvent breakEvent)
	{
		...
		String noBuildReason = GriefPreventionPlus.instance.allowBreak(player, block, block.getLocation());
	...
	public void onSignChanged(SignChangeEvent event)
	{
	    ...
	    if(!GriefPreventionPlus.instance.config_signNotifications) return;
		...
		if(!player.hasPermission("griefprevention.spam") && GriefPreventionPlus.instance.containsBlockedIP(signMessage)) {
...
*/
	...
	public void onBlockPlace(BlockPlaceEvent placeEvent)
	{
		...
		if(block.getType() == Material.FIRE && !GriefPreventionPlus.instance.config_pvp_enabledWorlds.contains(block.getWorld()) && !player.hasPermission("griefprevention.lava"))
		{
		...
		else if(block.getType() == Material.CHEST && GriefPreventionPlus.instance.config_claims_automaticClaimsForNewPlayersRadius > -1 && GriefPreventionPlus.instance.claimsEnabledForWorld(block.getWorld()))
		{			
			//if the chest is too deep underground, don't create the claim and explain why
			if(GriefPreventionPlus.instance.config_claims_preventTheft && block.getY() < GriefPreventionPlus.instance.config_claims_maxDepth)
			{
				GriefPreventionPlus.sendMessage(player, TextMode.Warn, Messages.TooDeepToClaim);
				return;
			}
			
			int radius = GriefPreventionPlus.instance.config_claims_automaticClaimsForNewPlayersRadius;
			...
			if(GriefPreventionPlus.instance.config_claims_preventTheft && this.dataStore.getClaimAt(block.getLocation(), false, playerData.lastClaim) == null)
			{
		...
		else if(block.getType() == Material.SAPLING && GriefPreventionPlus.instance.config_blockSkyTrees && GriefPreventionPlus.instance.claimsEnabledForWorld(player.getWorld()))
		{
		...
		else if(!this.trashBlocks.contains(block.getType()) && GriefPreventionPlus.instance.claimsEnabledForWorld(block.getWorld()))
		{
			   ...
			   || (playerData.lastClaim != null && playerData.lastClaim.isNear(player.getLocation(), 15))))
			{
			    ...
			    if(playerData.buildWarningTimestamp == null || now - playerData.buildWarningTimestamp > 600000)  //10 minute cooldown
			    {
		...
		if(	GriefPreventionPlus.instance.config_blockSurfaceOtherExplosions && block.getType() == Material.TNT &&
			block.getWorld().getEnvironment() != Environment.NETHER &&
			block.getY() > GriefPreventionPlus.instance.getSeaLevel(block.getWorld()) - 5 &&
			claim == null)
		{
	...
	{
	    //don't track in worlds where claims are not enabled
        if(!GriefPreventionPlus.instance.claimsEnabledForWorld(igniteEvent.getBlock().getWorld())) return;
        
	    
	    if(!GriefPreventionPlus.instance.config_fireSpreads && igniteEvent.getCause() != IgniteCause.FLINT_AND_STEEL &&  igniteEvent.getCause() != IgniteCause.LIGHTNING)
		{	
	...
	public void onBlockSpread (BlockSpreadEvent spreadEvent)
	{
		...
		if(!GriefPreventionPlus.instance.config_fireSpreads)
		{
	...
	{
	    //don't track in worlds where claims are not enabled
        if(!GriefPreventionPlus.instance.claimsEnabledForWorld(burnEvent.getBlock().getWorld())) return;
        
	    if(!GriefPreventionPlus.instance.config_fireDestroys)
		{
	...
	{
	    //don't track in worlds where claims are not enabled
        if(!GriefPreventionPlus.instance.claimsEnabledForWorld(dispenseEvent.getBlock().getWorld())) return;
		...
		if((materialDispensed == Material.WATER_BUCKET || materialDispensed == Material.LAVA_BUCKET) && GriefPreventionPlus.instance.creativeRulesApply(dispenseEvent.getBlock().getWorld()) && toClaim == null)
		{
    ...
    public void onTreeGrow (StructureGrowEvent growEvent)
    {
	    ...
	    if(!GriefPreventionPlus.instance.config_limitTreeGrowth) return;
	    
	    //don't track in worlds where claims are not enabled
        if(!GriefPreventionPlus.instance.claimsEnabledForWorld(growEvent.getWorld())) return;
