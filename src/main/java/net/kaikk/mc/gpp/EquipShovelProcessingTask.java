...
class EquipShovelProcessingTask implements Runnable 
{
	...
	public void run()
	{
		...
		if(playerData.shovelMode == ShovelMode.Basic)
		{
			...
			GriefPreventionPlus.sendMessage(player, TextMode.Instr, Messages.RemainingBlocks, String.valueOf(remainingBlocks));
			
			//link to a video demo of land claiming, based on world type
			if(GriefPreventionPlus.instance.creativeRulesApply(player.getWorld()))
			{
				GriefPreventionPlus.sendMessage(player, TextMode.Instr, Messages.CreativeBasicsVideo2, DataStore.CREATIVE_VIDEO_URL);			
			}
			else
			{
